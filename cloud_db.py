...
def migrate_imgbb_to_cloudinary():
    """Hàm tự động quét toàn bộ ảnh ImgBB cũ và đẩy lên Cloudinary."""
    questions = load_all_questions_from_cloud()
    if not questions:
        return 0, "Không có dữ liệu câu hỏi trên Google Sheet."
    ...
    if not (cloud_name and api_key and api_secret):
        return 0, "Chưa cấu hình đầy đủ CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET trong st.secrets!"
    ...
    for idx, q in enumerate(questions, start=1):
        ...
        # 1. Chuyển ảnh đề bài
        if q.image_path and "cloudinary.com" not in q.image_path and q.image_path.startswith("http"):
            try:
                res = requests.get(q.image_path, headers=headers, timeout=25)
                if res.status_code == 200 and len(res.content) > 100:
                    up_res = cloudinary.uploader.upload(
                        res.content,
                        folder="mathbank_images",
                        public_id=f"{q.code}_main",
                        overwrite=True,
                        resource_type="image"
                    )
                    q.image_path = up_res.get("secure_url") or up_res.get("url")
                    modified = True
            except Exception:
                pass

        # 2. Chuyển ảnh lời giải
        sol_img = getattr(q, 'solution_image_path', None)
        if sol_img and "cloudinary.com" not in sol_img and sol_img.startswith("http"):
            try:
                res = requests.get(sol_img, headers=headers, timeout=25)
                if res.status_code == 200 and len(res.content) > 100:
                    up_res = cloudinary.uploader.upload(
                        res.content,
                        folder="mathbank_images",
                        public_id=f"{q.code}_sol",
                        overwrite=True,
                        resource_type="image"
                    )
                    q.solution_image_path = up_res.get("secure_url") or up_res.get("url")
                    modified = True
            except Exception:
                pass
    ...
    if updated_count > 0:
        ...
        return updated_count, f"Đã chuyển đổi thành công {updated_count} câu hỏi sang Cloudinary!"
    return 0, "Tất cả ảnh đã ở Cloudinary từ trước hoặc không có ảnh ImgBB cần chuyển!"
